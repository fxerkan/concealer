...
The vault is also **cross-platform**: a vault created on macOS/Linux opens on **Windows** and vice-versa (same SOPS+age format). On Windows use `$env:CONCEALER_TOKEN`/`setx` and see the [Windows guide]({{ site.baseurl }}/WINDOWS.html) for the (minor) at-rest caveats.
